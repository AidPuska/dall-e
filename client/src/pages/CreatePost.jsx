...
const CreatePost = () => {
  ...
  const generateImg = async () => {
    if(form.prompt) {
      try {
        ...
        const response = await fetch('https://dall-e-aid.onrender.com//api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if(form.prompt &&  form.photo) {
      ...
      try {
        ...
        const res = await fetch('https://dall-e-aid.onrender.com//api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  return (
    <div className='w-full sm:p-8 px-4 py-8 bg-[#f9fafe] min-h-[calc(100vh-73px)]'>
        ...
        <form onSubmit={handleSubmit} className='mt-16 max-w-3xl'>
          <div className='flex flex-col gap-5'>
            ...
            <div className='relative bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-64 p-3 h-64 justify-center items-center'>
              ...
              {generatingImg && (
                <div className='absolute inset-2 z-0 flex justify-center items-center bg-black/40 rounded-lg'>
                  <Loader />
                </div>
              )}
            </div>
